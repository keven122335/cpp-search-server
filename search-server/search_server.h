...
public:
    ...
    std::vector<Document> FindTopDocuments(const std::string& raw_query, Predicat predicat) const {
        ...
        std::sort(matched_documents.begin(), matched_documents.end(), [EPSILON](const Document& lhs, const Document& rhs) {
...
private:
    ...
    bool correct(const std::string& word) const;
...
double ComputeWordInverseDocumentFreq(const std::string& word) const;
    ...
    template<typename Predicat>
    std::vector<Document> FindAllDocuments(const Query& query_words, Predicat predicat) const {
